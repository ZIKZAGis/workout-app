    ...
    return (
        <div className={cn(styles.row, {
            ...
            <div key={`RepeatWeight ${item.id}/${item.weight}`}>
                <input type="tel" pattern="[0-9]*" defaultValue={item.weight} disabled={item.isCompleted}/>
                <i>kg{item.isCompleted && ' '}</i>
            ...
            <div key={`Completed ${item.id}/${item.isCompleted}`}>
                <img 
                    src={item.isCompleted ? '/images/exercises/check-completed.svg' : '/images/exercises/check.svg'} 
